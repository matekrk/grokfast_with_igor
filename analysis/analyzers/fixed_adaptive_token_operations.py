...
class FixedAdaptiveTokenOperationDetector:
    ...
    def get_adaptive_threshold(self, operation_type, epoch, total_epochs, model_accuracy):
        ...
        if self.thresholds:
            threshold = self.thresholds.get_threshold(operation_type, epoch, total_epochs, model_accuracy)
        else:
            # Fallback thresholds - more lenient
            static_thresholds = {
                "copy": 0.3,  # ✅ FIX: Lower from 0.8
                "induction": 0.4,  # ✅ FIX: Lower from 0.7
                "component": 0.3
            }
...
class RegistrationAwareAdaptiveTokenOperationDetector(FixedAdaptiveTokenOperationDetector):
