...
# ============================================================================
# INTEGRATION EXAMPLE FOR DAY 4
# ============================================================================

# Usage in enhanced training loop:
"""
# In train_with_enhanced_circuit_analysis.py, add cross-level analysis:

# Cross-level circuit analysis (NEW in Week 3 Day 4)
if budget.can_run_method("cross_level_analysis"):
    cross_level_start = time.time()

    # Initialize cross-level detector
    cross_level_detector = CrossLevelCircuitDetector(
        model=model,
        registry=registry,
        save_dir=save_dir / "cross_level",
        logger=logger
    )

    # Run comprehensive cross-level analysis
    cross_level_results = cross_level_detector.analyze_cross_level_circuits(
        epoch=epoch,
    ...
    epoch_results["cross_level_results"] = cross_level_results

    cross_level_time = time.time() - cross_level_start
    budget.record_execution_time("cross_level_analysis", cross_level_time)

    # Log cross-level results
    logger.info(f"    🔗 Cross-level circuits: {cross_level_results['functional_circuits']['interactions_found']} functional, "
                f"{cross_level_results['sparse_feature_circuits']['interactions_found']} sparse feature, "
                f"{cross_level_results['representation_circuits']['interactions_found']} representation")
"""